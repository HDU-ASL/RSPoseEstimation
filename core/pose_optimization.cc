...
public:
    Cost_T(KDtree* kdtreeA, Matrix3X* originPoints,
               Eigen::Quaterniond stableRotateQ, const double* const point)
        : kdtreeA(kdtreeA), originPoints(originPoints), 
        stableRotateQ(stableRotateQ),point(point){}
    ...
    bool operator()(
        ...
        point_inverseT_stableT_tp[0] = ((Jet<double, 7>)j_point_inverseT_stableT_tp[0]).a;
        point_inverseT_stableT_tp[1] = ((Jet<double, 7>)j_point_inverseT_stableT_tp[1]).a;
        point_inverseT_stableT_tp[2] = ((Jet<double, 7>)j_point_inverseT_stableT_tp[2]).a;
    ...
    static ceres::CostFunction* Create(KDtree* const kdtreeA,
                                     ...
                                     Eigen::Quaterniond q,
                                     double * point) {
        return (new ceres::AutoDiffCostFunction<Cost_T, 3, 4, 3>(
            new Cost_T( kdtreeA, originPoints, q, point)));
    }
};
...
double getHightMiddleWithT(std::vector<double*> Points, 
    ...
    std::cout<<"t.data()[2]"<<t.data()[2]<<std::endl;
...
int main(int argc, char** argv) {
    ...
    bool flag = read_file(vertices_source, normal_source, src_vert_colors, file_source);
    ...
    cout << "source: " << originPoints.rows() << "x" << originPoints.cols()
            << "!!!" << std::endl;
    ...
    for (int i = 0; i < resultPoints.size(); ++i) {
		for( int j=1; j<=MAXSpoke; j++){
            ...
            Cost_T::Create(&kdtreeA, &originPoints, calulateQuaternionT(j,NAME_MAX),resultPoints[i]);
    ...
    // options.max_num_iterations = times;
